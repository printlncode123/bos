...
<script type="text/javascript">
	...
	function doAssociations(){
		...
		var rows=$("#grid").datagrid("getSelections");//获取选中的行，返回的是个数组
		if(rows.length!=1){
			...
			//发送ajax请求获取未关联到定区的客户信息，不刷新页面
			$.post("decidedzoneAction_findNotAssociation.action",function(data){
			...
			var decidedzone_id=rows[0].id;//需要传递的参数
			//发送ajax请求获取已经关联到指定定区(根据定区id)的客户信息，不刷新页面
			$.post("decidedzoneAction_findHasAssociation.action",{"id":decidedzone_id},function(data){
				for(var i=0;i<data.length;i++){
					var id=data[i].id;
					var name=data[i].name;
					var telephone=data[i].telephone;
	...
	function doDblClickRow(index,data){//index是双击行的索引，data是双击行的数据
		$('#association_subarea').datagrid( {
			fit : true,
			border : true,
			rownumbers : true,
			striped : true,
			url : "subareaAction_findListByDecidedzoneId?decidedzone.id="+data.id,//根据定区id查询关联的分区信息
			columns : [ [{
				field : 'id',
				title : '分拣编号',
				width : 120,
		...
		$('#association_customer').datagrid( {
			...
			rownumbers : true,
			striped : true,
			url : "decidedzoneAction_findHasAssociation.action?id="+data.id,//根据定区id查询关联的客户信息
			columns : [[{
				field : 'id',
				title : '客户编号',
				width : 120,
	...
	<div class="easyui-window" title="定区添加修改" id="addDecidedzoneWindow" collapsible="false" minimizable="false" maximizable="false" style="top:20px;left:200px">
		<div style="height:31px;overflow:hidden;" split="false" border="false" >
			<div class="datagrid-toolbar">
				...
				<script type="text/javascript">
					$("#save").click(function(){
						var v=$("#addDecidedzone").form("validate");//校验表单数据是否不为空
						if(v){
							$("#addDecidedzone").submit();//提交表单刷新页面
						}
					});
				</script>
			</div>
		...
		<div style="overflow:auto;padding:5px;" border="false">
			<form id="addDecidedzone" method="post" action="decidedzoneAction_add.action">
						...
						<td>
							...
							$(function(){
								...
								$("#associationBtn").click(function(){
									var rows=$("#grid").datagrid("getSelections");//获取选中的行，返回的是个数组
									var id=rows[0].id;
									$("input[id='customerDecidedZoneId']").val(id);
									//在表单提交前全选右边下拉框关联到定区的客户
									$("#associationSelect option").attr("selected","selected");
									$("#customerForm").submit();//提交表单刷新页面
								});
							});
